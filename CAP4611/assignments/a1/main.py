...
def q5_1():
    ...
    quantiles = np.quantile(X, [0.05, 0.25, 0.50, 0.75, 0.95])
    print("Quantiles:", quantiles)

    means = np.mean(X, axis=0)
    variances = np.var(X, axis=0)

    print("Highest mean:", names[np.argmax(means)])
    print("Lowest mean:", names[np.argmin(means)])
    print("Highest variance:", names[np.argmax(variances)])
    print("Lowest variance:", names[np.argmin(variances)])
