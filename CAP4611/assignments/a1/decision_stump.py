...
class DecisionStumpErrorRate:
    ...
    def predict(self, X):
        ...
        is_greater = X[:, self.j_best] > self.t_best

        y_hat[is_greater] = self.y_hat_yes
        y_hat[~is_greater] = self.y_hat_no
...
class DecisionStumpInfoGain(DecisionStumpErrorRate):
    ...
    def fit(self, X, y):
        ...
        # If all labels are the same, no split is needed
        if np.unique(y).size <= 1:
            return
        ...
        for j in range(d):
            for i in range(n):
                ...
                # Ignore splits where one side is empty
                if y_yes.size == 0 or y_no.size == 0:
                    continue

                count_yes = np.bincount(
                    y_yes,
                    minlength=num_classes
                )

                count_no = np.bincount(
                    y_no,
                    minlength=num_classes
                )

                p_yes = count_yes / y_yes.size
                p_no = count_no / y_no.size

                split_entropy = (
                    (y_yes.size / n) * entropy(p_yes)
                    + (y_no.size / n) * entropy(p_no)
                )
                ...
                if info_gain > max_gain:
                    ...
                    self.j_best = j
                    self.t_best = t
                    self.y_hat_yes = np.argmax(count_yes)
                    self.y_hat_no = np.argmax(count_no)
